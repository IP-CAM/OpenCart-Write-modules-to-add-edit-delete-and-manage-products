...
class ControllerUserApi extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('user/api', '' . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('user/api', '' . $url, true)
		);

		$data['add'] = $this->url->link('user/api/add', '' . $url, true);
		$data['delete'] = $this->url->link('user/api/delete', '' . $url, true);
		...
		foreach ($results as $result) {
			$data['apis'][] = array(
				...
				'edit'          => $this->url->link('user/api/edit', '' . '&api_id=' . $result['api_id'] . $url, true)
			);
		}
		...
		$data['sort_username'] = $this->url->link('user/api', '' . '&sort=username' . $url, true);
		$data['sort_status'] = $this->url->link('user/api', '' . '&sort=status' . $url, true);
		$data['sort_date_added'] = $this->url->link('user/api', '' . '&sort=date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->link('user/api', '' . '&sort=date_modified' . $url, true);
		...
		$pagination->url = $this->url->link('user/api', '' . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['api_id'])) {
			$data['action'] = $this->url->link('user/api/add', '' . $url, true);
		} else {
			$data['action'] = $this->url->link('user/api/edit', '' . '&api_id=' . $this->request->get['api_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('user/api', '' . $url, true);
