...
class ControllerUserUserPermission extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('user/user_permission', '' . $url, true));
		}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('user/user_permission/add', '' . $url, true);
		$data['delete'] = $this->url->link('user/user_permission/delete', '' . $url, true);
		...
		foreach ($results as $result) {
			$data['user_groups'][] = array(
				...
				'name'          => $result['name'],
				'edit'          => $this->url->link('user/user_permission/edit', '' . '&user_group_id=' . $result['user_group_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('user/user_permission', '' . '&sort=name' . $url, true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['user_group_id'])) {
			$data['action'] = $this->url->link('user/user_permission/add', '' . $url, true);
		} else {
			$data['action'] = $this->url->link('user/user_permission/edit', '' . '&user_group_id=' . $this->request->get['user_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('user/user_permission', '' . $url, true);
		...
		foreach ($files as $file) {
			...
			preg_match_all('/public function (\w+)/', file_get_contents($file), $match);

			foreach ($match[1] as $item) {
				$function[] = array(
					'name'  => $item,
					'value' => ($item == 'index') ? $permission : ($permission . '/' . $item)
				);
			}
		...
		if (isset($this->request->post['permission'])) {
			$data['access'] = $this->request->post['permission'];
		} elseif (isset($user_group_info['permission'])) {
			$data['access'] = $user_group_info['permission'];
		} else {
			$data['access'] = array();
		}
