...
class ControllerDesignTranslation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('design/translation', '' . $url, true));
		}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('design/translation/add', '' . $url, true);
		$data['delete'] = $this->url->link('design/translation/delete', '' . $url, true);
		...
		foreach ($results as $result) {
			$data['translations'][] = array(
				...
				'key'            => $result['key'],
				'value'          => $result['value'],
				'edit'           => $this->url->link('design/translation/edit', '' . '&translation_id=' . $result['translation_id'], true),
			);
		}
		...
		$data['sort_store'] = $this->url->link('design/translation', '' . '&sort=store' . $url, true);
		$data['sort_language'] = $this->url->link('design/translation', '' . '&sort=language' . $url, true);
		$data['sort_route'] = $this->url->link('design/translation', '' . '&sort=route' . $url, true);
		$data['sort_key'] = $this->url->link('design/translation', '' . '&sort=key' . $url, true);
		$data['sort_value'] = $this->url->link('design/translation', '' . '&sort=value' . $url, true);

		$pagination = new Pagination();
		$pagination->total = $translation_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('design/translation/history', '' . '&page={page}', true);
	...
	protected function getForm() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('design/translation', '' . $url, true)
		);

		if (!isset($this->request->get['translation_id'])) {
			$data['action'] = $this->url->link('design/translation/add', '' . $url, true);
		} else {
			$data['action'] = $this->url->link('design/translation/edit', '' . '&translation_id=' . $this->request->get['translation_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('design/translation', '' . $url, true);
