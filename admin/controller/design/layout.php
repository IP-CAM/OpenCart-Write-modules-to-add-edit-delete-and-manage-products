...
class ControllerDesignLayout extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('design/layout', '' . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('design/layout', '' . $url, true)
		);

		$data['add'] = $this->url->link('design/layout/add', '' . $url, true);
		$data['delete'] = $this->url->link('design/layout/delete', '' . $url, true);
		...
		foreach ($results as $result) {
			$data['layouts'][] = array(
				...
				'name'      => $result['name'],
				'edit'      => $this->url->link('design/layout/edit', '' . '&layout_id=' . $result['layout_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('design/layout', '' . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->link('design/layout', '' . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['layout_id'])) {
			$data['action'] = $this->url->link('design/layout/add', '' . $url, true);
		} else {
			$data['action'] = $this->url->link('design/layout/edit', '' . '&layout_id=' . $this->request->get['layout_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('design/layout', '' . $url, true);
		...
		foreach ($layout_modules as $layout_module) {
			...
			if (!isset($part[1])) {
				$data['layout_modules'][] = array(
					...
					'edit'       => $this->url->link('extension/module/' . $part[0], '', true),
				...
				if ($module_info) {
					$data['layout_modules'][] = array(
						...
						'code'       => $layout_module['code'],
						'edit'       => $this->url->link('extension/module/' . $part[0], '' . '&module_id=' . $part[1], true),
						'position'   => $layout_module['position'],
						'sort_order' => $layout_module['sort_order']
					);
				}				
