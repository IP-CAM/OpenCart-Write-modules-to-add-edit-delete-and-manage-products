...
class ControllerCustomerCustomField extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('customer/custom_field', '' . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('customer/custom_field', '' . $url, true)
		);

		$data['add'] = $this->url->link('customer/custom_field/add', '' . $url, true);
		$data['delete'] = $this->url->link('customer/custom_field/delete', '' . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['custom_fields'][] = array(
				...
				'edit'            => $this->url->link('customer/custom_field/edit', '' . '&custom_field_id=' . $result['custom_field_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('customer/custom_field', '' . '&sort=cfd.name' . $url, true);
		$data['sort_location'] = $this->url->link('customer/custom_field', '' . '&sort=cf.location' . $url, true);
		$data['sort_type'] = $this->url->link('customer/custom_field', '' . '&sort=cf.type' . $url, true);
		$data['sort_status'] = $this->url->link('customer/custom_field', '' . '&sort=cf.status' . $url, true);
		$data['sort_sort_order'] = $this->url->link('customer/custom_field', '' . '&sort=cf.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('customer/custom_field', '' . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['custom_field_id'])) {
			$data['action'] = $this->url->link('customer/custom_field/add', '' . $url, true);
		} else {
			$data['action'] = $this->url->link('customer/custom_field/edit', '' . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('customer/custom_field', '' . $url, true);
