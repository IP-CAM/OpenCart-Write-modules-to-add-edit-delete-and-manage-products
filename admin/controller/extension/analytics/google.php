...
class ControllerExtensionAnalyticsGoogle extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('marketplace/extension', '' . '&type=analytics', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', '' . '&type=analytics', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/analytics/google', '' . '&store_id=' . $this->request->get['store_id'], true)
		);

		$data['action'] = $this->url->link('extension/analytics/google', '' . '&store_id=' . $this->request->get['store_id'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', '' . '&type=analytics', true);
	...
	protected function validate() {
		if (!$this->user->hasPermission('extension/analytics/google')) {
