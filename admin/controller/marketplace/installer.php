...
class ControllerMarketplaceInstaller extends Controller {
	...
	public function history() {
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('marketplace/installer/history', '' . '&page={page}', true);
	...
	public function upload() {
		...
		if (!$json) {
			...
			if (is_file($file)) {
				...
				$json['next'] = str_replace('&amp;', '&', $this->url->link('marketplace/install/install', '' . '&extension_install_id=' . $extension_install_id, true));		
			} else {
